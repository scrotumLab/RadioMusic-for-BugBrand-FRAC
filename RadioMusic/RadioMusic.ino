/*
 ...
 Channel Pot: A9 // SLab inactive now - CHANNEL means PITCH
 Channel CV: A8  (original = A6)
 Time Pot: A7  // Slab inactive now - TIME means SAMPLE-SELECT
 Time CV: A6  (original = A8)
...
October 2018 by Scrotum Lab
- All changes affect pot - Now pot is mixed / att with CV IN. It does not go directly anymore to DAC.
- Main changes: 
-- pot commented
-- cv + pot combi changed
-- changed also txt file on card - Variables are now external - easier to experiment with note range and also set to BugBrand (-5v/+5v) or Eurorack mode (0v/+5v)
...
//		flashLeds = false;
    ...
    // SLab
    if((changes & (ROOT_NOTE_CHANGED | ROOT_CV_CHANGED) ) || resetTriggered) {
      speedChange = true;
    }
...
void doSpeedChange() {
  ...
  speed = interface.rootNote - settings.setRootNote;
  
	D(Serial.print("Root note ");Serial.println(interface.rootNote););
